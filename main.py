...
\end{equation}''')
    ...
    new_culture_absorptivity = st.number_input("", value=st.session_state.culture_absorptivity)
    st.latex(r'''Q_{radiation} = \sigma \cdot A \cdot e \cdot \left(T_{sky}^{4} - \left(T + 273.15 \right)^{4} \right)''')
    st.latex(r''' \scriptsize a \text{: Stefan-Boltzmann Constant, in } W/m^{2}/K^{4}''') 
    new_sigma = st.number_input("", value=st.session_state.sigma)
