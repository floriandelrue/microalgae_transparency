...
def calculate_optimum_transparency(your_loc, start_date_object, end_date_object, month, year, PAR_avg, raceway_area, depth,weather_data, weather_data_extended, X_initial, P_max, alpha, I_opt, T_min, T_opt, T_max,  kT, kI, C, K, nb_layer):
    ...
    ax2.text(0.5 * best_transparency, 0.75 * best_X, 'Best biomass concentration')
    ax2.text(0.65 * best_transparency, 0.8 * best_X, f'{best_X:.3f}', fontweight='bold', fontsize=15)
...
\end{equation}''')
    ...
    st.latex(r'''Q_{accumulated} = Q_{irradiance} + Q_{radiation} + Q_{evaporation}+ Q_{convection}+Q_{conduction}''')
    st.text("With:")
    st.latex(r'''Q_{accumulated} = depth \cdot A \cdot C_{p} \cdot \rho \cdot \frac{dT}{dt}''')
      
    st.latex(r''' \scriptsize depth \text{: Microalgae Culture Total Depth, in } m''')
    st.latex(r''' \scriptsize A \text{: surface area, in } m^{2}''')
    st.text("The microalgae reactor surface area has no impact on the temperature model results")
    st.latex(r''' \scriptsize C_{p} \text{: Specific Heat Capacity of the Culture, in } J/kg/°C''')
    new_C_p = st.number_input("", value=st.session_state.C_p)
    st.latex(r''' \scriptsize \rho \text{: Density of the Culture, in } kg/m^{3}''')
    new_rho = st.number_input("", value=st.session_state.rho)
    st.latex(r'''  \scriptsize T \text{: Temperature of the Culture, in } °C''')
    st.latex(r'''Q_{irradiance} = I_{W} \cdot a \cdot A''')
    st.latex(r''' \scriptsize a \text{: Culture absorptivity, no unit} ''')
...
if options.index(month_select) == 0:
  ...
  for month in range(1,13):
    ...
    weather_data_month = [var[hours:hours+num_hours_month] for var in weather_data]
    weather_data_month_extended = [var[hours : hours+num_hours_month+360] for var in weather_data_extended]
